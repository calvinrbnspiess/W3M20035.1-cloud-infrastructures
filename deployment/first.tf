...
resource "openstack_compute_instance_v2" "friedrich" {
  count           = 2
  name            = "friedrich-${count.index + 1}-name"
  image_id        = "c57c2aef-f74a-4418-94ca-d3fb169162bf"
  flavor_id       = "76a34987-433d-4d3d-be8f-84447b2e0a3e"
  key_pair        = "cloud-native-friedrich"
...
resource "local_file" "floating_ip" {
  depends_on = [openstack_compute_instance_v2.friedrich]

  content = join("\n", [
    for vm in openstack_compute_instance_v2.friedrich :
